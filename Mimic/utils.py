...
def save_checkpoint(dataFolderPath, encoder, decoder, epoch, cMean_tr, cStd_tr, vMean_tr, vStd_tr, aMean_tr, aStd_tr, lMean_tr, lStd_tr, dlMean_tr, dlStd_tr, curvatureLength, historyLength):
...
def MSE2wAtt(input, target, mean, std, alphas, alphas_target):
    vMSE = torch.mean((input[:, :, 0]-target[:, :, 0])**2)
    lMSE = torch.mean((input[:, :, 1]-target[:, :, 1])**2)
    MSE = vMSE + lMSE
    ATT = abs(torch.mean((alphas - alphas_target)))

    consistencyVA = 0  # Not tracked

    return MSE+100*ATT, (MSE, vMSE, lMSE, consistencyVA, 100*ATT)
