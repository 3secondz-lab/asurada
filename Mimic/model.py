...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        :param currentPosition: 현재 위치 0에서부터 얼마나 이동했는지, 이 값은 그냥 supervision으로 활용하고자 함. Not Used Here.
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, encoder_dim, lstm_input_dim, decoder_dim, attention_dim, output_dim):
        ...
        self.conv_carState = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=3, stride=1, padding=1, padding_mode='replicate')
    ...
    def concat_carState(self, carState):  # carState (B, 11, 2)

        carState = self.conv_carState(carState.permute(0, 2, 1))
        #pdb.set_trace()
        return carState.squeeze(1)
    ...
    def init_hidden_state(self, cEncOut, csEncOut):
        ...
        """
        mean_cEncOut = cEncOut.mean(dim=1)  # (1024, 32)
    ...
    def forward(self, cEncOut, curSpeeds, histSpeeds, curAccelXs, histAccelXs, curOffsets, histOffsets, decodeLength,
                vMean=None, vStd=None, aMean=None, aStd=None, lMean=None, lStd=None, dlMean=None, dlStd=None):
        """
        Forward propagation.

        :param cEncOut: encoded curvature vectors, a tensor of dimension (batch_size, 125, encoder_dim)
        :param curSpeeds: (batch_size) or (1)
        :param histSpeeds: (batch_size, historyLength)
        # :param curAccelXs: ()
        # :param histAccelXs:
        :param curOffsets: (batch_size, predLength+1)
        :param histOffsets: (batch_size, historyLength)
        :param decodeLength
        :return: scores
        """
        ...
        for t in range(decodeLength):
            ...
            # pdb.set_trace()
            # try:
            speed = torch.cat((histSpeeds, curSpeed.unsqueeze(1)), 1)  # (1024, 11)
            offset = torch.cat((histOffsets, curOffset.unsqueeze(1)), 1)  # (1024, 11)
            # except:
                # print('error')
                # pdb.set_trace()

            ## 여기서 차이.
            # AttentionModel_Master
            # AttentionModel_Master_LAT
            # AttentionModel_Master_LAT_conv
            # carState = speed
#            carState = torch.cat((speed, offset), 1)  # (1024, 22)  # 두개를 같은 dimension으로 cat을 안하면, cnn 같은 걸로 preprocessing을 해야 할거 같은데.. how?
            carState = torch.cat((speed.unsqueeze(-1), offset.unsqueeze(-1)), 2)  # (B, 11, 2)

            conv_carState = self.concat_carState(carState)

            if t == 0:
                # if not inferenceMode:  # inferenceMode면, 이미 위에서 계산된 h가 있음.
                # h, c = self.init_hidden_state(cEncOut, carState)
                h, c = self.init_hidden_state(cEncOut, conv_carState)

            ## h_t -> a_t -> c_t -> ~h_t
            # h, c = self.decode_step(carState, (h, c))
            h, c = self.decode_step(conv_carState, (h, c))
            ...
            curSpeed_t1 = curSpeed_t + (preds[:, 0]*aStd + aMean).squeeze(-1)
            currentPosition = currentPosition + (curSpeed_t + 0.5*(abs(curSpeed_t1-curSpeed_t)))/36  # 직진 distance
            ...
            if curSpeeds.dim() == 1:
                if batch_size == 1:  # test / simulation에서 batch_size가 1
                    # if t==0:
                        # pdb.set_trace()
                    curSpeed = (((curSpeed*vStd+vMean) + (preds[:, 0]*aStd + aMean)) - vMean)/vStd  # curSpeed update
                    # curSpeed = curSpeed.squeeze(-1)
                    curOffset = (((curOffset*lStd+lMean) + (preds[:, 1]*dlStd + dlMean)) - lMean)/lStd  # curOffset update
                    # curOffset = curOffset.squeeze(-1)
                else:
                    curSpeed = ((curSpeed*vStd+vMean).unsqueeze(-1) + (preds[:, 0]*aStd+aMean).unsqueeze(-1) - vMean) / vStd  # curSpeed update
                    curSpeed = curSpeed.squeeze(-1)
                    curOffset = (((curOffset*lStd+lMean) + (preds[:, 1]*dlStd + dlMean)) - lMean)/lStd  # curOffset update
                    curOffset = curOffset.squeeze(-1)
