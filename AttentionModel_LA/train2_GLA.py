...
vNumber = 2  # local attention with spatial information (model learns local attention point considering a guide (spatial information)
...
circuit_tr = ['mugello']  # multiple circuits -> ['mugello', 'imola', 'spa', ...]
circuit_vl = ['magione']

training_gt_rate = 0  # 1.1: 정답값으로만 학습, 0: 예측값으로만 학습
''' ======================================================================== '''
...
''' Model '''
encoder_dim = 32
lstm_input_dim = historyLength + 1  # concat(history, vt)
decoder_dim = 32  # hidden state
attention_dim = 32
output_dim = 1
criterion = MSEwAtt  #utils.py
...
def main():
    global epochs_since_improvement, best_loss_tr, checkpoint, start_epoch

    encoder = Encoder(encoder_dim)
    decoder = DecoderWithAttention(encoder_dim, lstm_input_dim, decoder_dim, attention_dim, output_dim, curvatureLength)
    ...
    print('predict len {}'.format(predLength))
    ...
    for epoch in tqdm(range(start_epoch, epochs)):
        ...
        if epochs_since_improvement > 0 and epochs_since_improvement % 20 == 0:
            adjust_learning_rate(epoch, encoder_optimizer, 0.95)
            adjust_learning_rate(epoch, decoder_optimizer, 0.95)
...
def train(trainLoader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (curvatures, targetSpeeds, histSpeeds, targetAccelXs, histAccelXs) in enumerate(trainLoader):
        ...
        if random() >= training_gt_rate:  # 모델이 학습하는 alphas와 alphas_target 사이 loss를 backpropagation
            predictions, alphas, alphas_target = decoder(curvatures, targetSpeeds[:, 0], histSpeeds, targetAccelXs, histAccelXs, decodeLength,
                                        ...
                                        trainLoader.dataset.aMean, trainLoader.dataset.aStd)  # 예측값 사용
        else:
        ...
        vCorr, aCorr = pearsonr(torch.cat((vPreds.unsqueeze(-1), aPreds.unsqueeze(-1)), 2), torch.cat((vTrues.unsqueeze(-1), aTrues.float().unsqueeze(-1)), 2))
...
def validate(validLoader, encoder, decoder, criterion):
    encoder.eval()
    decoder.eval()
