...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder_dim):
        ...
        self.conv3 = nn.Conv1d(in_channels=8, out_channels=encoder_dim, kernel_size=7, stride=1, padding=3, padding_mode='replicate')  # out_channels = encoder_dim
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden, currentPosition):
        ...
        :param currentPosition: 현재 위치 0에서부터 얼마나 이동했는지, 이 값을 attention point로 곧바로 사용
        ...
        # temp = temp - aligned_position  # gaussian distribution으로 만드려고
        temp = temp - currentPosition.unsqueeze(1)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, cEncOut, curSpeeds, histSpeeds, curAccelXs, histAccelXs, decodeLength,
        ...
        for t in range(decodeLength):
            ...
            currentPosition = currentPosition + ((curSpeed_t + 0.5*(abs(curSpeed_t1-curSpeed_t)))/36 * (numSubCurvatures/self.previewLength))
            # --> 0.1초 동안 가는 누적 거리 * encoded curvature length로 scale 보정
