...
def station_curvature(localX, localY, medfilt=15):  # Menger curvature
    dx = np.diff(localX)
    dy = np.diff(localY)
...
def getMapPreview(i, df, df_map, map_center_xy, previewDistance, transform=False, headingInfo=None):
    ...
    try:
        ...
        temp = (cur_idx_candi[0] - i)  # i를 뺀 값임. 왜냐하면, 맵을 빙글빙글 도니까, i 다음 index 가져오도록
...
def MSE(pred, target, mean, std):
    try:
        MSE = torch.mean((pred-target)**2)
    except:
        MSE = torch.mean((pred-target.float())**2)
    vMSE = 0
    aMSE = 0
    consistencyVA = 0
...
def MSEwAtt(pred, target, mean, std, alphas, alphas_target):
    try:
        MSE = torch.mean((pred-target)**2)
    except:
        MSE = torch.mean((pred-target.float())**2)
    ...
    return MSE+50*ATT, (MSE, vMSE, aMSE, consistencyVA, 100*ATT)
