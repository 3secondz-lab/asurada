...
class Attention(nn.Module):
    ...
    def __init__(self, encoder_dim, attention_dim, decoder_dim):
        ...
        self.window_width = 1  # 양 옆으로 2씩 보는데, 그러면 window 전체를 5, 실제 curvature에 mapping하면 10
    ...
    def forward(self, encoder_out, decoder_hidden, currentPosition):
        ...
        :param currentPosition: 현재 위치 0에서부터 얼마나 이동했는지, 이 값은 그냥 supervision으로 활용하고자 함 (이 함수 내에서는 사용x)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, cEncOut, curSpeeds, histSpeeds, curAccelXs, histAccelXs, decodeLength,
        ...
        alphas_target = torch.zeros(batch_size, decodeLength, numSubCurvatures).to(device)  # spatial information (예상 위치)
        ...
        for t in range(decodeLength):
            ...
            if t < (decodeLength-1):
                for b, position in enumerate(currentPosition):
                    try:
                        alphas_target[b, t+1, int(torch.round(position).item())] = 1
                    except:
