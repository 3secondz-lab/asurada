...
vNumber = 1  # local attention (model learns local attention point by itself)
...
''' Model '''
encoder_dim = 32
lstm_input_dim = historyLength + 1  # concat(historyVts, vt)
decoder_dim = 32  # hidden state
attention_dim = 32
output_dim = 1
criterion = MSE  # utils.py
...
def train(trainLoader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (curvatures, targetSpeeds, histSpeeds, targetAccelXs, histAccelXs) in enumerate(trainLoader):
        ...
        targetAccelXs = targetAccelXs.to(device)  # [0]: a_t 부터 예측 target
        ...
        loss, metrics = criterion(predictions, targets, mean, std)
