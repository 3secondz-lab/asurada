...
class TestEnv:
    def __init__(self, datafile, recFreq, cWindow=250.0, vpWindow=2, cUnit=10, vpUnit=10, condition=None, st=None, et=None, mapfile=None):
        ...
        self.targetLen = 2 * vpUnit  # vWindow * vUnit인데, 일단 걍 숫자로 ...
    ...
    def get_preview(self):
        for idx in range(len(self.df)):
            ...
            # yield할때는 curvature[1:]로 해야하나? -> prediction할때만 curvature[1:]로 하면 될 듯!
            ...
            if idx < self.histLen:
                ...
                histOffsets = self.df['lateralOffset'].iloc[idx-self.histLen:idx].values
            ...
            curOffset = self.df['lateralOffset'].iloc[idx]
            ...
            targetOffsets = self.df['lateralOffset'].iloc[idx+1:idx+self.targetLen+1].values
            ...
            yield curPosition, mapPreview, transMapPreview, dist, curvature, curSpeed, histSpeed, curAccelX, histAccelX, targetSpeeds, targetAccelXs, histOffsets, curOffset, targetOffsets
