...
def replicateMapInfo(mapFile, df, vis=False, startX=None, startY=None):
    ...
    for i in range(len(startingPoints)):
        ...
        dist += (df_map_dist + (last_dist_v+1)).tolist()  # normalization을 했을 때는 1m로 걍 더하면 됨.
        last_dist_v = dist[-1]
    dist = np.array(dist)
...
def save_checkpoint(dataFolderPath, encoder, decoder, epoch, cMean_tr, cStd_tr, vMean_tr, vStd_tr, aMean_tr, aStd_tr, lMean_tr, lStd_tr, curvatureLength, historyLength):
...
def weightedMseNslopeLoss(input, target):  # 사실은 weighted MSE
