...
if try1:
    ...
    PolyFit.training()
    # load
    # PolyFit.agent.load(70000)

    predict, abs_curvature, last_valid_idx = PolyFit.test(df_te, previewHelper_te, previewType)
    true = np.array([carSpeed[idx:idx+predict_length] for idx in range(last_valid_idx)])
    pdb.set_trace()
    # Graph
    for i in range(predict.shape[1]):
        fig, ax = plt.subplots()
        line1 = ax.plot(true[:, i], label='True', linewidth=2)
        line2 = ax.plot(predict[:, i], label='Predict', linewidth=2)

        ax1 = ax.twinx()
        line3 = ax1.plot([(-1)*x[i] for x in abs_curvature], 'k--', label='Curvature', linewidth=1)
